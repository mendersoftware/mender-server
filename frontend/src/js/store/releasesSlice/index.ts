...
type ReleaseSliceType = {
  ...
  deltaJobs: {
    byId: Record<string, EnhancedJobDetailsItem>;
    jobIds: string[];
    page: number;
    perPage: number;
    sort?: SortOptions;
...
export const releaseSlice = createSlice({
  ...
  reducers: {
    ...
    receivedDeltaJobs: (state, action) => {
      ...
      state.deltaJobs.byId = jobs.reduce(
        (accu, job) => {
          accu[job.id] = {
            ...accu[job.id],
            ...job
        ...
        { ...state.deltaJobs.byId }
      );
      state.deltaJobs.jobIds = jobs.map(job => job.id);
      state.deltaJobs.total = total;
    },
    receivedDeltaJobDetails: (state, action) => {
      const job = action.payload;
      state.deltaJobs.byId[job.id] = {
        ...state.deltaJobs.byId[job.id],
        ...job
      };
    },
    setDeltaJobsListState: (state, action) => {
      state.deltaJobs = { ...state.deltaJobs, ...action.payload };
    }
  }
});
