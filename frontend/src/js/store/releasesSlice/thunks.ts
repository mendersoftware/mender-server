...
export const getDeltaGenerationJobs = createAsyncThunk(`${sliceName}/getDeltaGenerationJobs`, (options = {}, { dispatch }) => {
  ...
  return GeneralApi.get(`${deploymentsApiUrlV2}/releases/all/delta/jobs?page=${page}&per_page=${perPage}${sortParam}`)
    .then(({ data, headers }) => {
...
export const getDeltaGenerationJobDetails = createAsyncThunk(`${sliceName}/getDeltaGenerationJobDetails`, (jobId, { dispatch }) =>
  GeneralApi.get(`${deploymentsApiUrlV2}/releases/all/delta/jobs/${jobId}`)
    .then(({ data }) => Promise.all([dispatch(actions.receivedDeltaJobDetails(data)), data]))
