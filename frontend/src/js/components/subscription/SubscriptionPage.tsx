...
type ContactReason = 'reduceLimit' | 'overLimit';
interface ContactReasonProps {
  reason: ContactReason;
}
const ContactReasonAlert = ({ reason }: ContactReasonProps) => {
  const reasonMap: { [key in ContactReason]: ReactNode | string } = {
    reduceLimit: `For over ${enterpriseDeviceCount} devices, please contact for pricing.`,
    overLimit: (
      <div>
...
export const SubscriptionPage = () => {
  ...
  useEffect(() => {
    if (!isOrgLoaded || selectedPlan.id === 'enterprise' || limit % 50 !== 0) {
      return;
    }
