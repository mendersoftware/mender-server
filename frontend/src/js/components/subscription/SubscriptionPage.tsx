...
import { Alert, Button, FormControl, FormControlLabel, Radio, RadioGroup, Typography, outlinedInputClasses } from '@mui/material';
...
import { ADDONS, Addon, AddonId, AvailableAddon, AvailablePlans, PLANS } from '@northern.tech/store/appSlice/constants';
...
const SubscriptionForm = ({ onShowUpgradeDrawer, onUpdateFormValues, previewPrice, setPreviewPrice, setOrder, specialHandling }: SubscriptionFormProps) => {
  ...
  useEffect(() => {
    if (limit < PLANS[selectedPlan.id].minimalDeviceCount) {
      setValue('limit', PLANS[selectedPlan.id].minimalDeviceCount);
    }
  }, [selectedPlan.id, limit, setValue]);
  ...
  }, [currentDeviceLimit, debouncedLimit, selectedPlan.minimalDeviceCount, selectedPlan.name, specialHandling]);

  useEffect(() => {
    if (!couldGetPreview) {
      return;
    }
    const effectiveLimit = Math.max(debouncedLimit, selectedPlan.minimalDeviceCount);
          ...
          <>
            ...
            <div className="flexbox center-aligned margin-top-x-small">
              <TextInput
                ...
                type="number"
                InputProps={{
                  inputProps: { min: selectedPlan.minimalDeviceCount, step: DIVISIBILITY_STEP },
                  size: 'small',
                  onBlur: handleDeviceLimitBlur
                }}
                hint={inputHelperText}
                width="100%"
              />
            </div>
          </>
