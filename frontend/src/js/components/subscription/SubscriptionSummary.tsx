...
export const SubscriptionSummary = (props: SubscriptionSummaryProps) => {
  ...
  return (
    <Card variant="outlined" className="padding" sx={{ minWidth: '320px' }}>
      <CardHeader
      ...
      />
      <CardContent className="padding-none" sx={{ '&:last-child': { pb: 0 } }}>
        <div className="flexbox space-between margin-top-small margin-bottom-small">
          ...
          <Typography variant="subtitle1">{isPreviewLoading ? <NumberSkeleton /> : currencyFormatter.format(previewPrice.plan / 100)}</Typography>
        </div>
            ...
            {enabledAddons.map(addon => (
              <div key={addon} className="flexbox space-between">
                ...
                <Typography variant="subtitle1">
                  {isPreviewLoading || !previewPrice.addons[addon] ? <NumberSkeleton /> : currencyFormatter.format(previewPrice.addons[addon] / 100)}
                </Typography>
              </div>
            ))}
          </div>
        ...
        <div className="flexbox space-between margin-top-small">
          ...
          <Typography variant="subtitle1">{isPreviewLoading ? <NumberSkeleton /> : currencyFormatter.format(previewPrice.total / 100)} </Typography>
        </div>
      </CardContent>
      {!readOnly && (
        <Button className="margin-top-small" sx={{ textTransform: 'none' }} disabled={!isNew} variant="contained" onClick={onAction} fullWidth>
          Upgrade now
        </Button>
      )}
    </Card>
