...
describe('SelfUserManagement Component', () => {
  ...
  it('changes the email through editUser on OS installations', async () => {
    ...
    await act(async () => vi.runAllTicks());
    await waitFor(() => expect(editUserSpy).toHaveBeenCalledWith(expect.objectContaining({ email: 'test@test.com', current_password: 'mysecretpassword' })));
  });
  ...
  it('initiates a verified email change on enterprise/hosted installations', async () => {
    ...
    await user.click(within(dialog).getByRole('button', { name: /confirm/i }));
    await act(async () => vi.runAllTicks());
    await waitFor(() => expect(initiateEmailChangeSpy).toHaveBeenCalledWith({ email: 'updated@example.com', current_password: 'mysecretpassword' }));
  });
});
