...
const getDefaultPhasesForPattern = (
  ...
  const minBatch = deploymentDeviceCount < phaseDefaults.batchSize && !filter ? Math.ceil((1 / deploymentDeviceCount) * 100) : phaseDefaults.batchSize;
  if (patternValue === rolloutPatterns.custom.key)
...
export const RolloutPatternSelection = ({ isEnterprise, disabledReason = '', previousPhases = [] }: RolloutPatternSelectionProps) => {
  ...
  const handlePatternChange = ({ target: { value } }) => {
    ...
    setValue(
      deploymentFormSections.phases,
      defaultPhases ?? (Array.isArray(value) ? structuredClone(value) : [{ batch_size: phaseLimits.fullBatchPercentage }])
    );
  };
  ...
  const onUsesPatternClick = useCallback(() => {
    ...
    setValue(deploymentFormSections.usesPattern, !usesPattern);
