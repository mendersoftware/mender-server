...
export const convertPhasesToMode = (phases, newMode: RolloutMode, numberDevices: number) =>
...
export const CustomPhaseTable = ({ filter, deploymentDeviceCount }: { deploymentDeviceCount: number; filter?: Filter }) => {
  ...
  const removePhase = index => {
    ...
    const { [batchKey]: _removed, delay, ...updatedLastPhase } = newPhases[newPhases.length - 1];
    if (newPhases.length > 1) {
      updatedLastPhase.delay = delay;
    }
    newPhases[newPhases.length - 1] = updatedLastPhase;
    setValue(deploymentFormSections.phases, newPhases);
  };
  ...
  const repeatPhase = (index: number) => {
    ...
    if (isPercentageMode && source[batchKey] >= evenSplitThreshold) {
      // distribute in 2 even phases instead of the regular handling
      setValue(deploymentFormSections.phases, [
        { ...source, [batchKey]: evenSplitThreshold },
        { ...duplicate, [batchKey]: evenSplitThreshold }
      ]);
      return;
    }
  ...
  const remainder = getRemainder({ phases, numberDevices: deploymentDeviceCount, rolloutMode });
        ...
        <TableCell>
          {!isLast && phases.length > 1 ? (
            <div className="flexbox">
              ...
              <IconButton disabled={phases.length <= 2} onClick={() => removePhase(index)} title="Remove phase">
                <CancelIcon />
              </IconButton>
            </div>
          ) : null}
