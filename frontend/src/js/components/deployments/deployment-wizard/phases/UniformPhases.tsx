...
import { phaseDefaults, phaseLimits, rolloutModes } from './constants';
import { computePhaseInfo, delayToSeconds, devicesToPercentage, getPhaseDeviceCount, parseInterval, percentageToDevices, useRowStyles } from './utils';
...
export const UniformPhaseSettings = ({ filter, deploymentDeviceCount }: { deploymentDeviceCount: number; filter?: Filter }) => {
  ...
  const batchDevices =
    uniformPhases?.batch_size_devices ??
    (isPercentageMode ? undefined : Math.min(consideredDevices || phaseLimits.fallbackDeviceCount, phaseLimits.maxDefaultBatchDevices));
  ...
  useEffect(() => {
    ...
    if (rolloutMode === rolloutModes.device_count.key) {
      ...
      const devices = current?.batch_size_devices || Math.min(consideredDevices || phaseLimits.fallbackDeviceCount, phaseLimits.maxDefaultBatchDevices);
  ...
  const handleBatchChange = (value: number) => {
    if (isPercentageMode) {
      updateUniformPhases(Math.min(phaseLimits.maxPerBatchPercentage, Math.max(1, value)), undefined);
    } else {
          ...
          <TableRow className={hasError ? classes.rowError : hasWarning ? classes.rowWarning : ''}>
            <TableCell>
              <BatchSizeInput
                ...
                max={isPercentageMode ? phaseLimits.maxPerBatchPercentage : maxDevices ? max : deploymentDeviceCount}
                hasError={hasError}
                messages={messages}
              />
            </TableCell>
