...
export const devicesToPercentage = (devices: number, numberDevices: number): number =>
  numberDevices > 0 ? Math.max(1, Math.min(phaseLimits.maxPerBatchPercentage, Math.round((devices / numberDevices) * 100))) : phaseDefaults.batchSize;
...
const getPercentagePhaseMessages = ({ phase, isLast, remainder, deploymentDeviceCount }: GetPhaseMessagesBaseProps): PhaseMessage[] => {
  ...
  if (batchSize != null && !isLast && (batchSize < 1 || batchSize > phaseLimits.maxPerBatchPercentage)) {
...
// truncating instead of rounding, so we never advertise more devices than a phase actually holds
