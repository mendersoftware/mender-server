...
const collectUsedLicenses = (startPath: string) => new Promise((resolve, reject) => init({ direct: 0, start: startPath }, (err, packages) => (err ? reject(err) : resolve(packages))));
...
const checkLicenses = (startPath: string) =>
  collectUsedLicenses(startPath)
...
const writeLicenseFile = (startPath: string) =>
  collectUsedLicenses(startPath)
    .then(simplifyPackageData)
    .then(async packages => {
