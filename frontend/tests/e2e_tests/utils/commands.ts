...
export const getStorageState = location => {
  ...
  try {
    ...
    const uniqueId = process.env.STAGING_ID ?? uuid();
    storageState = { username: process.env.STAGING_USER ?? `bot-test+${uniqueId}@northern.tech`, password: process.env.STAGING_PASSWORD ?? uuid(), uniqueId };
  }
...
export const startDockerClient = async (baseUrl, token) => {
  ...
  const args = [
    ...
    'mendersoftware/mender-client-docker-addons:client-main'
  ];
...
export const stopDockerClient = (): Promise<void> =>
  new Promise(resolve => {
    ...
    const child = spawn('docker stop connect-client', { shell: true });
...
export const tenantTokenRetrieval = async (baseUrl: string, page: Page) => {
  ...
  await page.waitForFunction(() => !!document.querySelector('.tenant-token-text')?.textContent);
