...
test.describe('SAML Login via sso/id/login', () => {
  ...
  test('Set up SAML', async ({ browserName, environment, baseUrl, loggedInPage: page, request }) => {
    ...
    const token = await getTokenFromStorage(baseUrl);
    const storedMetadataResponse = await request.get(`${baseUrl}api/management/v1/useradm/sso/idp/metadata`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const spMetadataResponse = await request.get(expectedSpMetaUrl);
