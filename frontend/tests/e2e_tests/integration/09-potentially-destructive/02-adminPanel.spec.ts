...
import { adminPanelApiUrl, findSecondaryTenant, getTenantDetail, listTenants } from '../../utils/adminPanel';
...
test.describe('Admin panel operator actions', () => {
  ...
  test('propagates a raised device limit through to the tenant', async ({ adminBaseUrl, baseUrl, password, request }) => {
    const tenant = await findSecondaryTenant(request, adminBaseUrl);
    const response = await request.put(adminPanelApiUrl(adminBaseUrl, `tenants/${tenant.id}/device-limits`), {
  ...
  test('locks a suspended tenant out and lets it back in again', async ({ adminBaseUrl, baseUrl, password, request }) => {
    const tenant = await findSecondaryTenant(request, adminBaseUrl);
    expect(await canLogIn(secondaryTenantUser, password, baseUrl, request)).toBeTruthy();

    const suspension = await request.put(adminPanelApiUrl(adminBaseUrl, `tenants/${tenant.id}/status`), { data: { status: 'suspended' } });
  ...
  test('moves a tenant onto a different plan', async ({ adminBaseUrl, request }) => {
    const tenant = await findSecondaryTenant(request, adminBaseUrl);
    expect(tenant.plan).toEqual('enterprise');

    try {
      const response = await request.put(adminPanelApiUrl(adminBaseUrl, `tenants/${tenant.id}`), { data: { plan: 'professional' } });
      expect(response.ok()).toBeTruthy();
      ...
      const { tenants: professional } = await listTenants(request, adminBaseUrl, { per_page: 100, plan: 'professional' });
      expect(professional.map(({ id }) => id)).toContain(tenant.id);
    } finally {
      const reset = await request.put(adminPanelApiUrl(adminBaseUrl, `tenants/${tenant.id}`), { data: { plan: tenant.plan } });
