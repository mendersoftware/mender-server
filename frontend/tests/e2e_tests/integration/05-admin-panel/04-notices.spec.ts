...
import { adminPanelApiUrl, adminPanelUrl, isAdminPanelAvailable } from '../../utils/adminPanel';
...
test.describe('Admin panel notices', () => {
  ...
  test.describe('composing', () => {
    test('previews what the recipients will see', async ({ baseUrl, page }) => {
      await page.goto(adminPanelUrl(baseUrl, 'notices'));
      ...
      await expect(preview.getByText(defaultTitle, { exact: true })).toBeVisible();
    });

    test('asks for a body before previewing anything', async ({ baseUrl, page }) => {
      await page.goto(adminPanelUrl(baseUrl, 'notices'));
      await page.getByRole('button', { name: /preview rendered email/i }).click();
    ...
    test('lets the operator take over the framing', async ({ baseUrl, page }) => {
      ...
      await expect(preview.getByText(defaultTitle, { exact: true })).toHaveCount(0);
    });

    test('will not broadcast until the confirmation is typed out', async ({ baseUrl, page }) => {
      await page.goto(adminPanelUrl(baseUrl, 'notices'));
      await page.getByLabel(/feature details/i).fill(noticeBody);
  ...
  test.describe('the plain text contract', () => {
    test('forwards the body verbatim instead of converting it', async ({ baseUrl, request }) => {
      const response = await request.post(adminPanelApiUrl(baseUrl, 'notices/preview'), { data: { body: noticeBody } });
      expect(response.ok()).toBeTruthy();
      const { text } = await response.json();

      // no markdown conversion happens anywhere in the pipeline
      expect(text).toContain(noticeBody);
      expect(text).not.toContain('<h1>');
      expect(text).not.toContain('<li>');
      expect(text).toContain(defaultTitle);
      expect(text).toContain(defaultGreeting);
    });

    test('refuses a notice without a body', async ({ baseUrl, request }) => {
      const response = await request.post(adminPanelApiUrl(baseUrl, 'notices/preview'), { data: { body: '' }, failOnStatusCode: false });
      expect(response.status()).toEqual(400);
    });

    test('refuses a test send without a recipient', async ({ baseUrl, request }) => {
      const response = await request.post(adminPanelApiUrl(baseUrl, 'notices/send'), {
    ...
    test('refuses a broadcast without the confirmation', async ({ baseUrl, request }) => {
      const response = await request.post(adminPanelApiUrl(baseUrl, 'notices/send'), {
  ...
  test.describe('sending', () => {
    test('delivers a test notice as plain text to the named address', async ({ baseUrl, environment, page, request, username }) => {
    ...
    test('broadcasts to the tenant admins across tenant boundaries', async ({ baseUrl, environment, request, username }) => {
      const emailClient = setupEmailClient(username, environment);
      test.skip(!emailClient, 'test requires a reachable mailbox');
