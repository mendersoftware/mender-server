...
import { adminPanelApiUrl, adminPanelUrl, findSecondaryTenant, getTenantDetail, isAdminPanelAvailable, tenantNames } from '../../utils/adminPanel';
...
test.describe('Admin panel tenants', () => {
  ...
  test.describe('list', () => {
    test('narrows the list down to a search term and back', async ({ baseUrl, page }) => {
      await page.goto(adminPanelUrl(baseUrl, 'tenants'));
    ...
    test('says so when nothing matches', async ({ baseUrl, page }) => {
      await page.goto(adminPanelUrl(baseUrl, 'tenants'));
    ...
    test('opens a tenant by clicking its row', async ({ baseUrl, page, request }) => {
  ...
  test.describe('detail form', () => {
    test('reveals the expiration only while the tenant is on trial', async ({ baseUrl, page, request }) => {
      const tenant = await findSecondaryTenant(request, baseUrl);
    ...
    test('keeps the subscription fields locked until they are unlocked', async ({ baseUrl, page, request }) => {
      const tenant = await findSecondaryTenant(request, baseUrl);
      await page.goto(adminPanelUrl(baseUrl, `tenants/${tenant.id}`));
    ...
    test('drops pending edits when they are cancelled', async ({ baseUrl, page, request }) => {
      const tenant = await findSecondaryTenant(request, baseUrl);
      await page.goto(adminPanelUrl(baseUrl, `tenants/${tenant.id}`));
    ...
    test('saves an edited tenant and reports back', async ({ baseUrl, page, request }) => {
      const tenant = await findSecondaryTenant(request, baseUrl);
      ...
      try {
        ...
        const restore = await request.put(adminPanelApiUrl(baseUrl, `tenants/${tenant.id}`), { data: { name: tenant.name } });
