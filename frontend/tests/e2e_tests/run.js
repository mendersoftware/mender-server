...
const collectClientLogs = async logDir => {
  ...
  const deploymentsLogs = await runCommand('ssh', ['-p', '8822', '-o', 'StrictHostKeyChecking=no', `root@${ip}`, 'cat', '/data/mender/deployment*.log'], {throwOnError: false, shell: true, ...config});
