...
const pollGateway = async (baseUrl, { maxAttempts = 10, delay = 2000 } = {}) => {
  const url = baseUrl || 'https://docker.mender.io';
  ...
  throw new Error(`Gateway at ${url} did not become reachable after ${maxAttempts} attempts`);
};
...
export const composeUp = async config => {
  ...
  await withSpinner(
    '⏳ Verifying gateway is reachable...',
    async () => await pollGateway(config.baseUrl),
    'Gateway is reachable',
    'Gateway failed to become reachable'
  );
};
