...
class NewDeviceInternalProvision(BaseModel):
    ...
    status: Optional[StrictStr] = Field(
        default=None, description="Authorization status for the device."
    )
    auth_sets: Optional[List[AuthSet]] = None
    created_ts: Optional[datetime] = Field(
        default=None, description="The creation timestamp of the device."
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(
            ["accepted", "noauth", "pending", "preauthorized", "rejected"]
        ):
            raise ValueError(
                "must be one of enum values ('accepted', 'noauth', 'pending', 'preauthorized', 'rejected')"
            )
        return value
