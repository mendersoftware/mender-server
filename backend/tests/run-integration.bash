    ...
    if [ "${CI_NODE_TOTAL:-1}" -gt 1 ]; then
        compose_cmd run $RUN_ARGS \
            ...
            integration-tester -c '
                ...
                PYTEST_ADDOPTS="" pytest --co -q -m "not slow" -k "not Enterprise and not Multitenant" 2>/tmp/co_err.txt | grep "::" | awk "NR % $CI_NODE_TOTAL == ($CI_NODE_INDEX - 1)" > /tmp/nodes
                echo "not-slow collected: $(wc -l < /tmp/nodes), co_err: $(wc -l < /tmp/co_err.txt)" >&2
                PYTEST_ADDOPTS="" pytest --co -q -m "slow"     -k "not Enterprise and not Multitenant" 2>>/tmp/co_err.txt | grep "::" | awk "NR % $CI_NODE_TOTAL == ($CI_NODE_INDEX - 1)" >> /tmp/nodes
                echo "total nodes: $(wc -l < /tmp/nodes)" >&2
                if [ -s /tmp/co_err.txt ]; then echo "collection stderr:" >&2; cat /tmp/co_err.txt >&2; fi
                [ -s /tmp/nodes ] || { echo "No tests for this shard"; exit 0; }
                mapfile -t nodes < /tmp/nodes && exec python3 -m pytest "${nodes[@]}"
            '
    else
