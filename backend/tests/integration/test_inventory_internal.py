...
class TestPatchAttributes:
    def test_inventory_patch_attributes_internal(self, clean_migrated_mongo):
        ...
        am = api.DeviceInventoryManagementAPIApi(api_client_inventory_management)
        for _ in redo.retrier(attempts=3, sleeptime=1):
            devices = am.list_device_inventories(
        ...
        for device in current_devices:
            if device.id == device_id_to_update:
                for attribute in device.attributes:
                    if attribute.name == "mac":
                        assert attribute.value.actual_instance == "de:ad:be:ef:06:12"
                        assert attribute.scope == "identity"
                        found = True
        assert found
        ...
        for device in current_devices:
            if device.id == device_id_to_update:
                for attribute in device.attributes:
                    if attribute.name == "mac":
                        assert attribute.value.actual_instance == "de:ad:be:ef:06:24"
                        assert attribute.scope == "identity"
                        found = True
        assert found
        ...
        for device in current_devices:
            if device.id == device_id_to_update:
                for attribute in device.attributes:
                    if attribute.name == "newone":
                        assert attribute.value.actual_instance == "brandnew"
                        assert attribute.scope == "system"
                        found = True
        assert found
