...
func (s *InventoryManagementV1Suite) newAcceptedDeviceWithSN(ctx context.Context) (*common.Device, error) {
	// a random "mac" identity attribute is added automatically; add "sn" on
	// top so the identity data carries both.
	sn := uuid.NewString()
	return common.NewAcceptedDevice(ctx, s.APIClient, s.Tenant.TenantToken,
		common.WithIdentityAttributes(map[string]any{"sn": sn}))
}
