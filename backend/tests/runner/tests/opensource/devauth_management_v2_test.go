...
// devauthKeyKinds lists the key kinds exercised by this suite.
var devauthKeyKinds = []string{
	common.KeyKindRSA,
	common.KeyKindECP224,
	common.KeyKindECP256,
	common.KeyKindECP384,
	common.KeyKindECP521,
	common.KeyKindEd25519,
}
...
func (s *DevauthManagementV2Suite) testPreauthOk() {
	...
	devs := make([]*common.Device, 0, len(devauthKeyKinds))
	for _, kind := range devauthKeyKinds {
		kp, err := common.NewKeyPair(kind)
		require.NoError(err)
		mac, err := common.RandomMAC()
		require.NoError(err)
		device, err := common.NewDevice(s.APIClient, s.Tenant.TenantToken,
			common.WithKeys(kp), common.WithMAC(mac.String()))
		require.NoError(err)
	...
	for _, device := range devs {
		...
		ok, err := device.SubmitAuthRequest(ctx)
		require.NoError(err)
		assert.True(ok)
	...
	for i, device := range devs {
		...
		require.NoError(err)
		fresh, err := common.NewDevice(s.APIClient, s.Tenant.TenantToken,
			common.WithKeys(kp), common.WithMAC(device.MAC))
		require.NoError(err)
		fresh.ID = device.ID
		devs[i] = fresh
	...
	for _, device := range devs {
		...
		for _, aset := range outdev.AuthSets {
			if aset.GetStatus() == model.DevStatusAccepted {
				foundAccepted = true
				assert.True(samePubkey(aset.GetPubkey(), device.Keys().ExportPublicKeyPEM()))
				break
			}
		}
...
func (s *DevauthManagementV2Suite) testPreauthFailDuplicate() {
	...
	assert.True(samePubkey(apiDev.AuthSets[0].GetPubkey(), kp.ExportPublicKeyPEM()))
	...
	for _, dev := range candidates {
		...
		if dev.GetStatus() == model.DevStatusAccepted {
			...
			for _, k := range keys[dev.GetId()] {
				if accepted.Pubkey != nil && samePubkey(*accepted.Pubkey, k.Keys().ExportPublicKeyPEM()) {
					device = k
					break
				}
			}
...
func (s *DevauthManagementV2Suite) testAuthsetMgmtPutStatusReject() {
	...
	for _, dev := range candidates {
		...
		if dev.GetStatus() == model.DevStatusAccepted {
			var device *common.Device
			for _, k := range keys[dev.GetId()] {
				if target.Pubkey != nil && samePubkey(*target.Pubkey, k.Keys().ExportPublicKeyPEM()) {
					device = k
					break
				}
			}
		...
		// no new auth set
		ok, err := targetDev.SubmitAuthRequest(ctx)
		require.NoError(err)
		assert.False(ok)
...
func (s *DevauthManagementV2Suite) testAuthsetMgmtDeleteStatus() {
	...
	for _, dev := range devs {
		...
		if dev.GetStatus() == model.DevStatusPreauth {
			// Removing the preauth auth set (the device's only auth set)
			// removes the device entirely. That removal is asynchronous,
			// driven by the workflows service, and the whole-device
			// decommission cascade is exercised by the decommission suite.
			// Here we only assert the synchronous contract of the
			// delete-auth-set endpoint (the 204 above). devices are listed
			// sorted by status ascending (accepted, pending, preauthorized,
			// rejected), so the first preauthorized device is the last one
...
func (s *DevauthManagementV2Suite) createPending(ctx context.Context, kind string, numAuthsets int) ([]*common.Device, error) {
	...
	for i := 0; i < numAuthsets; i++ {
		...
		if _, err := device.SubmitAuthRequest(ctx); err != nil {
			return nil, err
		}
...
func (s *DevauthManagementV2Suite) createAccepted(ctx context.Context, kind string, numAuthsets, numAccepted int) ([]*common.Device, error) {
	...
	for i := 0; i < numAccepted; i++ {
		pubkey := authSets[i].Keys().ExportPublicKeyPEM()
...
func (s *DevauthManagementV2Suite) createPreauthdWithPending(ctx context.Context, kind string, numPending int) ([]*common.Device, error) {
	...
	for i := 0; i < numPending; i++ {
		...
		device, err := common.NewDevice(s.APIClient, s.Tenant.TenantToken,
			common.WithKeys(kp), common.WithMAC(mac))
		if err != nil {
			return nil, err
		}
		device.ID = id
