...
// scopes its listing assertions (by exact name, or by a
// created_after/created_before time window) rather than relying on exact,
// unscoped totals.
...
func (s *DeploymentsManagementV1Suite) TestRegularDeploymentAllDevices() {
	// An all-devices deployment targets every previously accepted device in
	// the shared, never-reset environment, not just the ones this test
	// created, so assert stats.pending against the accepted-device count
	// observed right before creating the deployment plus the devices created
	// here.
	require := require.New(s.T())
	assert := assert.New(s.T())
	ctx := common.JWTAuthContext(s.T().Context(), s.JWT)
...
func (s *DeploymentsManagementV1Suite) TestListingDeployments() {
	// The deployments v2 list Name(...) filter accepts only a single name in
	// the generated client, so "list all of them" style assertions are scoped
	// to a created_after/created_before window bracketing this test.
	require := require.New(s.T())
	assert := assert.New(s.T())
	ctx := common.JWTAuthContext(s.T().Context(), s.JWT)
	...
	// created_after/created_before are inclusive, second-grained bounds. Since
	// another test may have created a deployment in the same wall-clock
	// second, wait for a fresh second boundary before recording "before" so it
	...
	// This test runs on the host while the server runs in the compose network;
	// on macOS the docker VM clock can drift. Measure the skew from the first
	// deployment's server-assigned created timestamp and shift all window
	// bounds by it, so created_after/created_before compare on the server
	// clock.
	var clockSkew int32
	...
	// invalid per_page/page: 400. The generated client types per_page/page as
	// int32, so use values that are valid int32s but still rejected by the API
	// (per_page over its configured max, page below 1).
...
// itself (JWT-authenticated), reporting artifactName/deviceType as the
// currently installed artifact and device type. It returns the raw response
// so callers can assert on the status code (200 update available, 204 nothing
// to deploy).
func (s *DeploymentsManagementV1Suite) checkUpdate(
	ctx context.Context, dev *common.Device, artifactName, deviceType string,
) (*client.DeploymentInstructions, *http.Response, error) {
	return s.APIClient.DeploymentsDeviceAPIAPI.
...
// tryUpdate asks for the next update for the device and, if one is offered,
// immediately reports it as successfully installed. It returns the CheckUpdate
// status code.
func (s *DeploymentsManagementV1Suite) tryUpdate(
	ctx context.Context, dev *common.Device, artifactName, deviceType string,
) (int, error) {
	instr, r, err := s.checkUpdate(ctx, dev, artifactName, deviceType)
	if r == nil {
		return 0, err
	}
...
// zero, and that the mentioned ones match.
...
// statusUpdateAndVerify updates a device's deployment status (optionally with a
// substate) and checks the resulting device-deployment and deployment status.
...
func (s *DeploymentsManagementV1Suite) statusVerify(
	...
	if expectDeviceStatus != "" {
		...
		require.NoError(err)
		// A device that isn't part of the deployment simply isn't in the list;
		// the not-part-of-deployment status-update case relies on asserting
	...
	if expectDeploymentStatus != "" {
		dep, _, err := s.APIClient.DeploymentsManagementAPIAPI.ShowDeployment(ctx, deploymentID).Execute()
		require.NoError(err)
...
func (s *DeploymentsManagementV1Suite) runDeploymentStatusUpdate(deployToGroup string) {
	...
	_, r2, err := s.checkUpdate(ctx, devs[0], "bugs-bunny", deviceType)
	require.NoError(err)
	require.Equal(http.StatusOK, r2.StatusCode)

	// devs[1]: already has the artifact installed
	_, r2, err = s.checkUpdate(ctx, devs[1], artifactName, deviceType)
	require.NoError(err)
	...
	_, r2, err = s.checkUpdate(ctx, devs[2], "bugs-bunny", "foo")
	require.NoError(err)
	require.Equal(http.StatusNoContent, r2.StatusCode)
	...
	// downloading -> pause_before_installing: device status and deployment
	// stats reflect the pause substate.
	...
	// downloading -> pause_before_committing
	...
	// pause_before_committing -> pause_before_rebooting
	...
	// mid-cycle re-poll: CheckUpdate again with the original artifact/device-type
	// params still returns the same deployment while it's inprogress.
	instrAgain, r2b, err := s.checkUpdate(ctx, devs[0], "bugs-bunny", deviceType)
	require.NoError(err)
	...
	_, r2, err = s.checkUpdate(ctx, devs[3], "bugs-bunny", deviceType)
	require.NoError(err)
	require.Equal(http.StatusOK, r2.StatusCode)
	...
	// post-finish override: reporting a new terminal status for a device whose
	// deployment has already finished is still accepted; the deployment status
	// stays "finished" but the stats reflect the override.
	assert := assert.New(s.T())
