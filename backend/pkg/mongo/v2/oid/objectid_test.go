...
func TestUUID(t *testing.T) {
	...
	assert.Equal(t, bson.TypeBinary, bson.Type(bsonType))

	// Illegal bsontype
	err = uRandom.UnmarshalBSONValue(byte(bson.TypeUndefined), b)
	assert.Error(t, err)
	// OK
	err = uRandom.UnmarshalBSONValue(bsonType, b)
	assert.NoError(t, err)
	// Invalid sub-type
	b[4] = bson.TypeBinaryGeneric
	err = uRandom.UnmarshalBSONValue(bsonType, b)
	assert.Error(t, err)
	// Invalid length
...
func TestBSONID(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, bson.TypeObjectID, bson.Type(bsonType))
	assert.NotEmpty(t, b)

	// Unmarshal with wrong bsontype
	tmp := &ObjectID{}
	err = tmp.UnmarshalBSONValue(byte(bson.TypeBinary), b)
	assert.Error(t, err)
	// Unmarshal OK
	err = tmp.UnmarshalBSONValue(byte(bson.TypeObjectID), b)
	assert.NoError(t, err)
	assert.Equal(t, id, *tmp)
	// Unmarshal malformed data
	err = tmp.UnmarshalBSONValue(byte(bson.TypeObjectID), []byte{})
	assert.Error(t, err)
...
func TestString(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, bson.TypeString, bson.Type(bsonType))
	err = tmp.UnmarshalBSONValue(byte(bson.TypeUndefined), b)
	assert.Error(t, err)
