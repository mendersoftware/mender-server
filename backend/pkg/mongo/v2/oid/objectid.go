...
func NewBSONID() ObjectID {
	return ObjectID{id: bson.NewObjectID()}
}
...
func fromString(id string) interface{} {
	...
	case 24: // Hex-encoded bson-type objectID type.
		mgoID, e := bson.ObjectIDFromHex(id)
		if e != nil {
...
func marshalBSONUUID(u uuid.UUID) (bson.Type, []byte, error) {
...
func (oid ObjectID) MarshalBSONValue() (byte, []byte, error) {
	...
	case uuid.UUID:
		...
		return byte(t), b, err
	case bson.ObjectID:
		t, b, err := bson.MarshalValue(objectID)
		return byte(t), b, err
	case string:
		t, b, err := bson.MarshalValue(objectID)
		return byte(t), b, err
	}
	return byte(bson.TypeNull), nil, errors.New(
...
func (oid *ObjectID) UnmarshalBSONValue(t byte, b []byte) error {
	...
	default:
		return errors.Errorf(
			"illegal bson-type %s, expected ObjectID", bson.Type(t),
		)
	}
	return err
}
