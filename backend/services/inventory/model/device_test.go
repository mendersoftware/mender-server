...
func TestValidateGroupName(t *testing.T) {
	...
	assert.EqualError(t, rules.DeviceGroupeName(group1), "the length must be no more than 256")
	group2 := GroupName("totally.legit")
	assert.EqualError(t, rules.DeviceGroupeName(group2), "Group name can only contain: "+
		"upper/lowercase alphanum, -(dash), _(underscore)")
	group3 := GroupName("")
	assert.EqualError(t, rules.DeviceGroupeName(group3), "cannot be blank")
	group4 := GroupName("test")
	assert.NoError(t, rules.DeviceGroupeName(group4))
}
