...
const (
	...
	urlInternalAttributesNoScope = "/tenants/:tenant_id/device/:device_id/attribute"
...
func (i *InternalAPI) PatchDeviceAttributesInternalHandler(
	...
	scopeFromUrl := true
	r := strings.NewReplacer(":tenant_id", tenantId, ":device_id", deviceId)
	if strings.Contains(
		c.Request.RequestURI,
		r.Replace(urlInternalAttributesNoScope),
	) {
		scopeFromUrl = false
	}
	for i := range attrs {
