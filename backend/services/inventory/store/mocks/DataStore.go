...
// GetDeviceTierStatisticsByStatus provides a mock function with given fields: ctx
func (_m *DataStore) GetDeviceTierStatisticsByStatus(ctx context.Context) (*model.DeviceStatisticsByStatus, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for GetDeviceTierStatisticsByStatus")
	}

	var r0 *model.DeviceStatisticsByStatus
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) (*model.DeviceStatisticsByStatus, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) *model.DeviceStatisticsByStatus); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.DeviceStatisticsByStatus)
		}
	}
...
func (_m *DataStore) SearchDevicesByIdentity(ctx context.Context, searchParams model.SearchIdentityParams) ([]model.Device, int, error) {
	ret := _m.Called(ctx, searchParams)
