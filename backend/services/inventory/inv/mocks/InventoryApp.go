...
// GetDeviceStatistics provides a mock function with given fields: ctx
func (_m *InventoryApp) GetDeviceStatistics(ctx context.Context) (*model.DeviceStatistics, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for GetDeviceStatistics")
	}

	var r0 *model.DeviceStatistics
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) (*model.DeviceStatistics, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) *model.DeviceStatistics); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.DeviceStatistics)
		}
	}
