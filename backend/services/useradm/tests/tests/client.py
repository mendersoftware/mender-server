...
class InternalApiClient:
    ...
    def verify(self, token, uri="/api/management/1.0/auth/verify", method="POST"):
        if not token.startswith("Bearer "):
            token = "Bearer " + token
        result, status, headers = self.client.verify_jwt_with_http_info(
            authorization=token,
            x_forwarded_uri=uri,
    ...
    def create_tenant(self, tenant_id):
        ...
        result, status, headers = self.client.useradm_create_tenant_with_http_info(tenant_new=tenant)
        return Response(status_code=status, data=result)

    def create_user_for_tenant(self, tenant_id, user):
        result, status, headers = self.client.create_user_internal_with_http_info(
...
class ManagementApiClient:
    ...
    def create_user(self, user, auth=None):
        ...
        if auth:
            ...
            if token.startswith("Bearer "):
                self.client.api_client.configuration.access_token = token[7:]
        result, status, headers_resp = self.client.create_user_management_with_http_info(user_new=user)
        return Response(status_code=status, data=result)
    ...
    def update_user(self, uid, update, auth=None):
        ...
        if auth:
            ...
            if token.startswith("Bearer "):
                self.client.api_client.configuration.access_token = token[7:]
        result, status, headers_resp = self.client.update_user_with_http_info(id=uid, user_update=update)
        return Response(status_code=status, data=result)
    ...
    def logout(self, auth=None):
        ...
        if auth:
            ...
            if token.startswith("Bearer "):
                self.client.api_client.configuration.access_token = token[7:]
        result, status, headers_resp = self.client.logout_with_http_info()
        return Response(status_code=status, data=result)
    ...
    def create_token(self, token_request, auth=None):
        ...
        if auth:
            ...
            if token.startswith("Bearer "):
                self.client.api_client.configuration.access_token = token[7:]
        result, status, headers_resp = self.client.create_personal_access_token_with_http_info(personal_access_token_request=token_request)
        # result is the token string
        return Response(status_code=status, text=result)
    ...
    def list_tokens(self, auth=None):
        ...
        if auth:
            ...
            if token.startswith("Bearer "):
                self.client.api_client.configuration.access_token = token[7:]
        result, status, headers_resp = self.client.list_user_personal_access_tokens_with_http_info()
        return Response(status_code=status, data=result)
