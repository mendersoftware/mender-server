...
func (_m *DataStore) GetUserByEmail(ctx context.Context, email user.Email) (*model.User, error) {
	...
	var r0 *model.User
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, user.Email) (*model.User, error)); ok {
		return rf(ctx, email)
	}
	if rf, ok := ret.Get(0).(func(context.Context, user.Email) *model.User); ok {
		r0 = rf(ctx, email)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.User)
