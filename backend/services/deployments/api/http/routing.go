...
func NewRouter(
	...
	NewDeploymentsResourceRoutes(router.Group("."), deploymentsHandlers)
...
func InternalRoutes(router *gin.RouterGroup, controller *DeploymentsApiHandlers) {
	...
	// Health Check
	router.GET(ApiUrlInternalAlive, controller.AliveHandler)
	router.GET(ApiUrlInternalHealth, controller.HealthHandler)
