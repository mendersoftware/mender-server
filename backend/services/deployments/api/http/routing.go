...
func NewRouter(
	...
	routerHealthz := router.Group("", accesslogSupressed)
	routerOpen := router.Group("", accesslogDefault)
	routerAuth := router.Group("", accesslogDefault, identity.Middleware())

	NewImagesResourceRoutes(routerAuth, deploymentsHandlers, cfg)
