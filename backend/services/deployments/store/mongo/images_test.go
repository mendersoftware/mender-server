...
func TestListV2Images(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"ok, by multiple name prefixes": {
			filter: &model.ImageFilter{
				NamePrefixes: []string{"App1", "App2"},
			},
			images: []*model.Image{
				inputImgs[0],
				inputImgs[2],
		...
		"ok, by combination of exact and prefix names ": {
			filter: &model.ImageFilter{
				ExactNames:   []string{"App1 v1.0"},
				NamePrefixes: []string{"App2"},
			},
			images: []*model.Image{
				inputImgs[0],
				inputImgs[2],
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			images, count, err := ds.ListImagesV2(ctx, tc.filter)
