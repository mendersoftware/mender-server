	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			for _, tenantId := range tc.TenantIDs {
				...
				_, err := c.InsertOne(ctx, mongostore.WithTenantID(
					ctx,
					model.Device{
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("tc %d", i), func(t *testing.T) {
			...
			if tc.outErr != "" {
				...
				err := c.FindOne(ctx, mongostore.WithTenantID(ctx, bson.M{"_id": tc.id})).Decode(&found)
...
func TestStoreDeleteDevice(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("tc %d", i), func(t *testing.T) {
			...
			for _, d := range inputDevices {
				_, err := c.InsertOne(dbCtx, mongostore.WithTenantID(ctx, d))
				assert.NoError(t, err, "failed to setup input data")
			}
...
func TestDeleteLimit(t *testing.T) {
	...
	var lim model.Limit
	assert.NoError(t, coll.FindOne(dbCtx, mongostore.WithTenantID(dbCtxOtherTenant, bson.M{dbFieldName: lim1.Name})).Decode(&lim))
	...
	// limit not found
	assert.Error(t, coll.FindOne(dbCtx, mongostore.WithTenantID(dbCtx, bson.M{dbFieldName: lim1.Name})).Decode(&lim))
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("tc %d", i), func(t *testing.T) {
			...
			for _, a := range authSets {
				_, err := coll.InsertOne(dbCtx, mongostore.WithTenantID(ctx, a))
				assert.NoError(t, err)
			}
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("tc %d", i), func(t *testing.T) {
			...
			if tc.err != nil {
				...
				coll := db.client.Database(DbName).Collection(DbAuthSetColl)
				err := coll.FindOne(ctx, mongostore.WithTenantID(ctx, bson.M{"_id": tc.aid})).Decode(&found)
				assert.NoError(t, err)
