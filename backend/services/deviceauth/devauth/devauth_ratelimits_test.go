...
func TestCheckRateLimits(t *testing.T) {
	...
	t.Run("ok/token bucket", func(t *testing.T) {
		...
		d := new(DevAuth).WithRatelimits(rateLimiter, make(map[string]float64))
	...
	t.Run("error/unknown error propagated", func(t *testing.T) {
		...
		d := new(DevAuth).WithRatelimits(errLimiter{err: errExpected}, map[string]float64{})
