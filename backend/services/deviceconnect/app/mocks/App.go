...
// GetShutdownNotification provides a mock function with no fields
func (_m *App) GetShutdownNotification() (<-chan struct{}, context.CancelFunc) {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetShutdownNotification")
	}

	var r0 <-chan struct{}
	var r1 context.CancelFunc
	if rf, ok := ret.Get(0).(func() (<-chan struct{}, context.CancelFunc)); ok {
		return rf()
	}
	if rf, ok := ret.Get(0).(func() <-chan struct{}); ok {
		r0 = rf()
...
// Shutdown provides a mock function with given fields: ctx, gracePeriod
func (_m *App) Shutdown(ctx context.Context, gracePeriod time.Duration) error {
