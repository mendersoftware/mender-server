...
func TestDeviceConnectDeadPeerReaped(t *testing.T) {
	...
	// Simulate an unresponsive peer: the client's TCP stack stays alive and
	// ACKs the server's pings, but it never pongs, sends data, or closes at
	// the application layer. Without the read deadline that leaves the server
	// no signal and it blocks in ReadMessage; with it the connection is reaped
	// within ~pongWait.
	select {
	case <-disconnected:
...
func TestDeviceConnectShutdown(t *testing.T) {
	prevPongWait := pongWait
	prevWriteWait := writeWait
	defer func() {
		pongWait = prevPongWait
	...
	app.On("GetShutdownNotification").Return((<-chan struct{})(shutdown), context.CancelFunc(func() {}))
	app.On("SetDeviceConnected",
		mock.MatchedBy(func(_ context.Context) bool { return true }),
		Identity.Tenant,
		Identity.Subject,
	...
	var shutdownErr *websocket.CloseError
	if assert.ErrorAs(t, err, &shutdownErr) {
		assert.Equal(t, websocket.CloseGoingAway, shutdownErr.Code, "unexpected shutdown close code")
	}
